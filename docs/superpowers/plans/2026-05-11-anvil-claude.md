# Anvil for Claude Code — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a single superpowers skill file (`skills/anvil.md`) that implements the full Anvil evidence-first coding agent loop for Claude Code, plus the plugin scaffold needed to distribute it.

**Architecture:** A rigid skill markdown file defines the Anvil persona, 9-phase loop, pushback protocol, SQLite evidence ledger (`.anvil/checks.db`), adversarial subagent review, and Evidence Bundle format. `plugin.json` and `.mcp.json` provide the plugin manifest. The skill is invoked via the `Skill` tool as `anvil` and is built incrementally — one section per commit.

**Tech Stack:** Markdown (skill file), JSON (plugin manifest), SQLite via `sqlite3` CLI through Bash, Claude Code native tools (TaskCreate/TaskUpdate, Agent, AskUserQuestion, Read, Edit, Write, Bash, mcp__ide__getDiagnostics)
...
- Create: `implementations/.gitkeep`
...
```json
{
  "name": "anvil",
  "version": "1.0.0",
  "description": "Evidence-first coding agent for Claude Code. Verifies every change with adversarial multi-agent review, SQL-tracked verification, and automatic rollback.",
  "author": {
    "name": "Adeel Nayyer",
    "url": "https://github.com/addeelnayyer"
  },
  "homepage": "https://github.com/addeelnayyer/anvil",
  "repository": "https://github.com/addeelnayyer/anvil",
  "license": "MIT",
  "keywords": ["verification", "code-review", "adversarial", "evidence", "claude-code"],
  "skills": "skills/",
  "mcpServers": ".mcp.json"
}
```
...
```bash
mkdir -p skills implementations
touch skills/.gitkeep implementations/.gitkeep
```

- [ ] **Step 5: Verify files exist**

```bash
ls -1 plugin.json .mcp.json .gitignore skills/.gitkeep implementations/.gitkeep
```
...
```bash
git add plugin.json .mcp.json .gitignore skills/.gitkeep implementations/.gitkeep
git commit -m "chore: add plugin scaffold (plugin.json, .mcp.json, .gitignore)"
```

---

### Task 2: Skill File — Frontmatter, Persona, Task Sizing, Phase Index

**Files:**
- Create: `skills/anvil.md`

- [ ] **Step 1: Create skills/anvil.md**

Write the following as the full initial content of `skills/anvil.md`:
...
```bash
git add skills/anvil.md
git commit -m "feat: add anvil skill header, persona, task sizing, and phase index"
```

---

### Task 3: Phases 0a + Pushback Protocol

**Files:**
- Modify: `skills/anvil.md` — append two new sections to the end

- [ ] **Step 1: Append Phase 0a — Boost**

Open `skills/anvil.md` and append to the end:
...
- [ ] **Step 2: Append Pushback Protocol**

Continue appending to `skills/anvil.md`:
...
- [ ] **Step 3: Verify both sections were appended**

```bash
grep -n "Phase 0a\|Pushback Protocol" skills/anvil.md
```
Expected: both headers appear with line numbers.

- [ ] **Step 4: Commit**

```bash
git add skills/anvil.md
git commit -m "feat: add Phase 0a (Boost) and Pushback Protocol"
```

---

### Task 4: Phases 0b, 1, 2, 3, 3b — Git Hygiene Through Plan

**Files:**
- Modify: `skills/anvil.md` — append five new sections

- [ ] **Step 1: Append Phase 0b — Git Hygiene**
...
- [ ] **Step 2: Append Phase 1 — Understand**
...
- [ ] **Step 6: Verify all five sections**

```bash
grep -n "^## Phase 0b\|^## Phase 1\|^## Phase 2\|^## Phase 3\b\|^## Phase 3b" skills/anvil.md
```
Expected: all 5 headers found with line numbers.

- [ ] **Step 7: Commit**

```bash
git add skills/anvil.md
git commit -m "feat: add phases 0b through 3b (Git Hygiene, Understand, Recall, Survey, Plan)"
```

---

### Task 5: Phase 4 — Baseline + SQLite Bootstrap

**Files:**
- Modify: `skills/anvil.md` — append Phase 4 section

- [ ] **Step 1: Append Phase 4 — Baseline**
...
```bash
grep -n "Phase 4\|sqlite3\|anvil_checks\|Hard gate" skills/anvil.md | head -20
```
Expected: Phase 4 header, multiple sqlite3 references, anvil_checks table name, and at least 2 Hard gate markers.

- [ ] **Step 3: Commit**

```bash
git add skills/anvil.md
git commit -m "feat: add Phase 4 (Baseline) with SQLite bootstrap and INSERT pattern"
```

---

### Task 6: Phase 5 — Implement

**Files:**
- Modify: `skills/anvil.md` — append Phase 5 section

- [ ] **Step 1: Append Phase 5 — Implement**
...
```bash
grep -n "^## Phase 5" skills/anvil.md
```
Expected: Phase 5 header found with line number.

- [ ] **Step 3: Commit**

```bash
git add skills/anvil.md
git commit -m "feat: add Phase 5 (Implement)"
```

---

### Task 7: Phase 6 — The Forge (Verification Cascade + Adversarial Review)

**Files:**
- Modify: `skills/anvil.md` — append Phase 6 section (largest section)

- [ ] **Step 1: Append Phase 6 header and 6a–6b**
...
- [ ] **Step 2: Append 6c — Adversarial Review**
...
git restore .
```
Then report: what was attempted, what failed, what each reviewer found. Never present broken code.
```
...
- [ ] **Step 4: Verify all Phase 6 subsections**

```bash
grep -n "^### 6a\|^### 6b\|^### 6c\|^### 6d\|^### 6e\|Fix loop" skills/anvil.md
```
Expected: all 6 markers found.

- [ ] **Step 5: Verify reviewer names are consistent**

```bash
grep -n "security-reviewer\|correctness-reviewer\|performance-reviewer" skills/anvil.md
```
Expected: each reviewer name appears at least twice.

- [ ] **Step 6: Commit**

```bash
git add skills/anvil.md
git commit -m "feat: add Phase 6 (The Forge) — cascade, adversarial review, fix loop"
```

---

### Task 8: Phases 7–9 + Hard Gates + Error Handling

**Files:**
- Modify: `skills/anvil.md` — append final sections

- [ ] **Step 1: Append Phase 7 — Learn**
...
- [ ] **Step 6: Verify all final sections**

```bash
grep -n "^## Phase 7\|^## Phase 8\|^## Phase 9\|^## Hard Gates\|^## Error Handling" skills/anvil.md
```
Expected: all 5 section headers found.

- [ ] **Step 7: Final line count**

```bash
wc -l skills/anvil.md
```
Expected: 350+ lines.
...
```bash
git add skills/anvil.md
git commit -m "feat: add phases 7-9, hard gates, and error handling — skill complete"
```
...
**Files:**
- Modify: `skills/anvil.md` — only if fixes are needed

- [ ] **Step 1: Check for placeholder text**

```bash
grep -in "tbd\|todo\|fill in\|placeholder\|xxx\|implement later\|coming soon" skills/anvil.md
```
Expected: no output. If any found, fix them before continuing.

- [ ] **Step 2: Verify all 12 phase markers are present**

```bash
grep -n "^## Phase 0a\|^## Phase 0b\|^## Phase 1\|^## Phase 2\|^## Phase 3\b\|^## Phase 3b\|^## Phase 4\|^## Phase 5\|^## Phase 6\|^## Phase 7\|^## Phase 8\|^## Phase 9" skills/anvil.md
```
Expected: all 12 lines found.

- [ ] **Step 3: Verify all Hard Gates are present**

```bash
grep -c "Hard gate\|Hard Gate" skills/anvil.md
```
Expected: 8 or more (one per gate in the Hard Gates table plus inline references).

- [ ] **Step 4: Verify sqlite3 commands are complete**

```bash
grep -c "sqlite3" skills/anvil.md
```
Expected: 6 or more (bootstrap CREATE, baseline INSERT, after INSERT, review INSERT, regression query, Evidence Bundle SELECT).

- [ ] **Step 5: Verify reviewer names are consistent throughout**

```bash
grep -c "security-reviewer" skills/anvil.md
grep -c "correctness-reviewer" skills/anvil.md
grep -c "performance-reviewer" skills/anvil.md
```
Expected: each count is 2 or more.

- [ ] **Step 6: Verify Pushback Protocol is referenced from Phase 0a**

```bash
grep -n "Pushback Protocol" skills/anvil.md
```
Expected: at least 2 lines (the section header and at least one reference from Phase 0a or 3b).

- [ ] **Step 7: Verify frontmatter is valid**

```bash
head -6 skills/anvil.md
```
Expected:
```
---
...
- [ ] **Step 8: Commit fixes if any were made**

```bash
git status
# Only if skills/anvil.md was modified:
git add skills/anvil.md
git commit -m "fix: skill self-review corrections"
```

- [ ] **Step 9: Final git log**

```bash
git log --oneline
```
Expected: at least 9 commits from scaffold through this review.
